...
![Node singularity: the curve y² = x²(x+1) self-intersects at the origin](figures/node-singularity.png)
*A **node** (crunode): the curve $y^2 = x^2(x+1)$ has a self-intersection at the origin, where two smooth branches cross with distinct tangent lines $y = \pm x$. Multiplicity 2 at the origin.*

![Cusp singularity: the curve y² = x³](figures/cusp-singularity.svg)
*A **cusp**: the curve $y^2 = x^3$ (semicubical parabola) has a single branch pinched to a point at the origin, with one tangent line $y = 0$ of multiplicity 2. The curve is smooth away from the origin.*
...
![Tangent line touching a curve](figures/tangent-intersection.svg)
*A line tangent to a curve at a point $P$ contributes intersection multiplicity $\geq 2$ at $P$: the two "intersection points" merge. A transverse crossing yields multiplicity 1. The intersection number $(C \cdot D)_P$ measures this contact order algebraically.*
...
![Bézout's theorem: a conic and a cubic meeting in 6 points](figures/bezout-theorem.svg)
*Bézout's theorem illustrated: a conic (degree 2) and a cubic (degree 3) in $\mathbb{P}^2$ meet in $2 \cdot 3 = 6$ points counted with multiplicity. Tangencies contribute intersection number $\geq 2$ at the tangent point.*
...
![Blow-up of the affine plane at the origin](figures/blowup.png)
*The blow-up $\mathrm{Bl}_0 \mathbb{A}^2$: the origin is replaced by the exceptional divisor $E \cong \mathbb{P}^1$ (red), which parametrizes all directions through the origin. Lines through the origin become separated in the blow-up; their strict transforms (gray) each meet $E$ at the corresponding direction point.*
