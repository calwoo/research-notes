...
The Riemann-Roch theorem and Hurwitz's formula are not merely theoretical — they drive concrete computational methods in coding theory, cryptography, and computational algebraic geometry.

> [!INFO] Algebraic geometry codes (Goppa codes)
> The key idea: for a divisor $D$ on a curve $X$ and a set of points $\mathcal{P} = \{P_1,\ldots,P_m\} \subset X \setminus \mathrm{supp}(D)$, the *algebraic geometry code* $C(\mathcal{P}, D)$ is the image of the evaluation map $\mathrm{ev}: H^0(\mathcal{O}(D)) \to k^m$, $f \mapsto (f(P_1),\ldots,f(P_m))$. Riemann-Roch controls the dimension and minimum distance.

> [!QUESTION] Exercise 19: Dimension of an AG code
> *This problem derives the key parameters of an algebraic geometry code from Riemann-Roch.*
>
> > **Prerequisites:** [[#4.2 Statement and Proof Sketch|4.2 Statement and Proof Sketch]], [[#4.3 Key Corollaries|4.3 Key Corollaries]]
>
> Let $X$ be a smooth projective curve of genus $g$ over $\mathbb{F}_q$. Let $D$ be a divisor with $2g - 2 < \deg D < m = |\mathcal{P}|$ (i.e., in the "non-special" range). Write Python pseudocode to: (a) compute $\ell(D) = \deg D + 1 - g$ (using Corollary 2), and (b) compute the minimum distance bound $d \geq m - \deg D$ (from the fact that a nonzero codeword has at most $\deg D$ zeros). Combine to state the parameters $[m, k, d]$ of $C(\mathcal{P}, D)$.

> [!TIP]- Solution to Exercise 19
> **Key insight:** Riemann-Roch in the non-special range gives $k = \ell(D) = \deg D + 1 - g$, and the minimum distance satisfies $d \geq m - \deg D$ by a pole-count argument.
>
> **Sketch:**
> ```python
> def ag_code_parameters(g: int, deg_D: int, m: int) -> dict:
>     """
>     Compute parameters of AG code C(P, D) via Riemann-Roch.
>     Assumes 2g - 2 < deg_D < m (non-special range, evaluation well-defined).
>     """
>     assert deg_D > 2 * g - 2, "D must be non-special (deg D > 2g-2)"
>     assert deg_D < m, "deg D must be less than number of evaluation points"
>
>     # Riemann-Roch (Corollary 2): ell(D) = deg(D) + 1 - g
>     k = deg_D + 1 - g   # dimension of code = dimension of H^0(O(D))
>
>     # Minimum distance: a nonzero f in H^0(O(D)) has at most deg(D) zeros,
>     # so ev(f) has at least m - deg(D) nonzero coordinates.
>     d_min = m - deg_D   # Goppa bound
>
>     return {"length": m, "dimension": k, "min_distance_bound": d_min}
>
> # Example: genus-2 curve, deg D = 6, m = 20 evaluation points
> params = ag_code_parameters(g=2, deg_D=6, m=20)
> # k = 6 + 1 - 2 = 5, d >= 20 - 6 = 14 => [20, 5, 14] code
> ```
> The Singleton bound gives $d \leq m - k + 1 = m - \deg D + g$; AG codes achieve $d \geq m - \deg D$, which is $g$ short of the Singleton bound — but far better than random codes when $g$ is small.

> [!QUESTION] Exercise 20: Computing the genus from branch data
> *This problem uses Hurwitz's formula as an algorithm to compute the genus of a curve presented as a branched cover.*
>
> > **Prerequisites:** [[#6.2 Ramification Divisor and Hurwitz|6.2 Ramification Divisor and Hurwitz]]
>
> Write Python pseudocode that takes as input: the degree $n$ of a cover $f: X \to \mathbb{P}^1$, the base genus $g_Y = 0$, and a list of ramification data `[(e_1, count_1), (e_2, count_2), ...]` where each pair represents `count_i` ramification points with ramification index `e_i`. Compute: (a) $\deg R = \sum_i \mathrm{count}_i \cdot (e_i - 1)$ and (b) $g(X)$ via Hurwitz. Validate against the hyperelliptic case ($n=2$, $2g+2$ branch points each with $e=2$).

> [!TIP]- Solution to Exercise 20
> **Key insight:** Hurwitz gives $g(X) = 1 - n + \tfrac{\deg R}{2}$ for covers of $\mathbb{P}^1$.
>
> **Sketch:**
> ```python
> def genus_from_hurwitz(
>     n: int,
>     g_Y: int,
>     ramification_data: list[tuple[int, int]]
> ) -> int:
>     """
>     Compute g(X) for f: X -> Y of degree n via Hurwitz's formula.
>
>     Args:
>         n: degree of the cover
>         g_Y: genus of base curve Y
>         ramification_data: list of (e_P, count) pairs; e_P = ramification index
>
>     Returns:
>         g(X) as an integer (assumes deg R is even, i.e., tame ramification)
>     """
>     deg_R = sum((e - 1) * count for e, count in ramification_data)
>     # Hurwitz: 2g(X) - 2 = n*(2*g_Y - 2) + deg_R
>     two_g_minus_2 = n * (2 * g_Y - 2) + deg_R
>     assert two_g_minus_2 % 2 == 0, "deg R parity error"
>     g_X = (two_g_minus_2 + 2) // 2
>     return g_X
>
> # Hyperelliptic validation: n=2, g_Y=0, 2g+2 branch points with e=2
> for g_target in range(1, 6):
>     branch_count = 2 * g_target + 2
>     g_computed = genus_from_hurwitz(n=2, g_Y=0, ramification_data=[(2, branch_count)])
>     assert g_computed == g_target, f"Failed for g={g_target}"
>     print(f"g={g_target}: {branch_count} branch points -> g(X) = {g_computed} ✓")
> ```

> [!QUESTION] Exercise 21: Checking smoothness via the Jacobian criterion
> *This problem encodes the Jacobian smoothness test and connects it to the rank-1 property of Kähler differentials.*
>
> > **Prerequisites:** [[#2.1 The Module of Kähler Differentials|2.1 The Module of Kähler Differentials]]
>
> Let $C = V(f) \subset \mathbb{A}^2$ for a polynomial $f \in k[x,y]$. Write Python pseudocode that, given $f$ as a sympy expression in `x, y`, computes the Jacobian $(\partial f/\partial x, \partial f/\partial y)$, evaluates it at each affine point of $C$ over a finite field $\mathbb{F}_q$, and flags points where the Jacobian vanishes as *singular*. Explain why vanishing of the Jacobian at a point $P \in C$ is equivalent to $\Omega_{k[x,y]/(f)/k}$ failing to be locally free of rank 1 at $P$.

> [!TIP]- Solution to Exercise 21
> **Key insight:** The Jacobian criterion for smoothness is the infinitesimal version of the Kähler differential rank-1 condition: $\mathrm{Spec}(A/\mathfrak{m})$-rank of $\Omega_{A/k}$ equals the Jacobian rank.
>
> **Sketch:**
> ```python
> import sympy as sp
...
>
> def find_singular_points(f_expr, q: int):
>     """
>     Find singular points of C = V(f) over F_q via the Jacobian criterion.
>     A point P is singular iff f(P) = 0 and (df/dx)(P) = (df/dy)(P) = 0.
>     """
>     x, y = sp.symbols('x y')
>     f = f_expr
>     df_dx = sp.diff(f, x)
...
>         if (int(f.subs([(x, a), (y, b)])) % q == 0 and
>                 int(df_dx.subs([(x, a), (y, b)])) % q == 0 and
>                 int(df_dy.subs([(x, a), (y, b)])) % q == 0):
>             singular.append((a % q, b % q))
>     return singular
>
> # Example: nodal cubic y^2 - x^3 - x^2 has a node at (0,0)
> x, y = sp.symbols('x y')
> f = y**2 - x**3 - x**2
> print(find_singular_points(f, q=7))  # should include (0, 0)
> ```
> At $P$: $\Omega_{A/k} \otimes_A k \cong k^2 / \langle (\partial f/\partial x)(P), (\partial f/\partial y)(P) \rangle$. If the Jacobian $(\nabla f)(P) = 0$, this quotient has dimension 2 (not 1), so $\Omega_{A/k}$ is not locally free of rank 1 at $P$ — the curve is singular there.

> [!QUESTION] Exercise 22: Riemann-Roch dimension via the Brill-Noether approach
> *This problem encodes the computation of $\ell(D)$ for an effective divisor on an explicit curve over $\mathbb{F}_q$ using a basis of global sections.*
>
> > **Prerequisites:** [[#4.2 Statement and Proof Sketch|4.2 Statement and Proof Sketch]]
>
> Let $X$ be an elliptic curve $E: y^2 = x^3 + ax + b$ over $\mathbb{F}_q$ (with $a, b \in \mathbb{F}_q$). Let $D = n \cdot O$ where $O = [0:1:0]$ is the identity. Using the Weierstrass basis $\{1, x, y, x^2, xy, \ldots\}$ from §5.2, write Python pseudocode to: (a) list a basis for $H^0(E, \mathcal{O}(nO))$ as monomials in $x, y$, subject to the relation $y^2 = x^3 + ax + b$; and (b) verify that the dimension equals $n$ for $n \geq 1$.

> [!TIP]- Solution to Exercise 22
> **Key insight:** Monomials $x^i y^j$ with $j \in \{0,1\}$ and pole order $\leq n$ at $O$ form a basis; pole order of $x^i y^j$ is $2i + 3j$.
>
> **Sketch:**
> ```python
> def weierstrass_basis(n: int) -> list[tuple[int,int]]:
>     """
>     Compute a basis for H^0(E, O(n*O)) on a Weierstrass elliptic curve.
>     Uses the fact that x has a double pole and y has a triple pole at O,
>     and that {x^i * y^j : j in {0,1}, 2i + 3j <= n} is a basis.
>
>     Returns list of (i, j) exponents for x^i * y^j.
>     """
>     basis = []
>     for j in range(2):         # j in {0, 1} (use y^2 = x^3 + ax + b to reduce)
...
>     return basis
>
> # Verify dimension equals n for n >= 1
> for n in range(1, 8):
>     basis = weierstrass_basis(n)
>     dim = len(basis)
>     print(f"n={n}: basis={basis}, dim={dim}, expected={n}")
>     assert dim == n, f"Dimension mismatch at n={n}"
> ```
> For $n = 1$: $\{(0,0)\}$ i.e., $\{1\}$, dim 1 ✓. For $n = 2$: $\{(0,0),(1,0)\}$ i.e., $\{1, x\}$, dim 2 ✓. For $n = 3$: $\{1, x, y\}$, dim 3 ✓. The pattern matches the Riemann-Roch prediction $\ell(nO) = n$ for $n \geq 1$ on a genus-1 curve.

> [!QUESTION] Exercise 23: Finding Weierstrass points
> *This problem encodes the search for Weierstrass points — points $P$ where $\ell(gP) > 1$ — via linear algebra over the function field.*
>
> > **Prerequisites:** [[#5.2 Genus 1: Weierstrass Form from Riemann-Roch|5.2 Genus 1: Weierstrass Form from Riemann-Roch]], [[#4.3 Key Corollaries|4.3 Key Corollaries]]
>
> A point $P$ on a genus-$g$ curve $X$ is a *Weierstrass point* if $\ell(gP) > 1$ (equivalently, there exists a rational function on $X$ with a pole of order $\leq g$ at $P$ and no other poles). Every curve of genus $g \geq 1$ has finitely many Weierstrass points; their count (with multiplicity) is $g^3 - g$ for $g \geq 2$.
>
> (a) For genus $g = 2$: $\ell(2P) > 1$ iff $2P \sim K$ (the canonical divisor). Using this characterization, describe an algorithm to find Weierstrass points on a genus-2 curve $y^2 = f(x)$ (degree-6 squarefree $f$). (b) Write Python pseudocode sketching the search for such points over $\mathbb{F}_q$.

> [!TIP]- Solution to Exercise 23
> **Key insight:** For genus 2, $\ell(2P) > 1$ iff $P$ is a branch point of the hyperelliptic involution, since $2P \sim K$ iff $P$ is a ramification point of $f: X \to \mathbb{P}^1$ (the Weierstrass points are exactly the $2g+2 = 6$ ramification points).
>
> **Sketch:** (a) $\ell(2P) = \ell(K - 2P) + \deg(2P) + 1 - 2 = \ell(K-2P) + 1$. So $\ell(2P) > 1$ iff $\ell(K - 2P) > 0$ iff $K \sim 2P$ (since $\deg(K - 2P) = 0$, so $\ell(K-2P) > 0$ iff $K \sim 2P$). The canonical divisor of $y^2 = f(x)$ (genus 2) is represented by the ramification divisor of $x: X \to \mathbb{P}^1$. The Weierstrass points are the $2(2)+2 = 6$ ramification points, i.e., the 6 roots of $f$.
>
> ```python
> def weierstrass_points_genus2(f_coeffs: list, q: int) -> list:
>     """
>     Find Weierstrass points on y^2 = f(x) (genus-2 hyperelliptic) over F_q.
>     Weierstrass points = ramification points of x: X -> P^1
>                        = zeros of f (points with y=0) plus infinity if f has even degree.
>     f_coeffs: coefficients of f in increasing degree, e.g. [a0, a1, ..., a6] for degree 6.
>     """
>     weierstrass = []
>     for a in range(q):
>         # Evaluate f at x = a over F_q
>         val = sum(c * pow(a, i, q) for i, c in enumerate(f_coeffs)) % q
>         if val == 0:
>             weierstrass.append((a, 0))  # branch point (a, 0) on y^2 = f(x)
>     # For degree-6 f: infinity is unramified; for degree 5: add infinity
>     return weierstrass
> ```
