...
> $$\pi_0(\mathbb{S}) = \pi_0(B(\mathbf{B}\Sigma)) = \pi_0(\Omega B(\mathbf{B}\Sigma)_1).$$
> The group completion of $\pi_0|\hat{\Sigma}(\mathbf{1})| = \mathbb{N}$ is $\mathbb{Z}$, consistent with $\pi_0(\mathbb{S}) = \mathbb{Z}$. **The BPQ theorem promotes this $\pi_0$-level computation to a full equivalence of spectra.**
>
> **(d)** For Proposition 3.6: $\mathbf{B}\Sigma_X(\mathbf{1}) = \bigsqcup_{n \geq 0} E\Sigma_n \times_{\Sigma_n} X^n$, so
> $$\pi_0(\mathbf{B}\Sigma_X(\mathbf{1})) = \bigsqcup_{n \geq 0} \pi_0(X^n/\Sigma_n) = \bigsqcup_{n \geq 0} \mathrm{Sym}^n(\pi_0 X),$$
> the free commutative monoid on $\pi_0 X$. The group completion is the free abelian group $\mathbb{Z}[\pi_0 X]$, which equals $\pi_0(\Sigma^\infty X_+) = \tilde{h}_0(\Sigma^\infty X_+)$ — the $0$th stable homotopy group of $X$ with a disjoint basepoint, which is indeed $\mathbb{Z}[\pi_0 X]$ by the Hurewicz theorem in stable homotopy theory.
