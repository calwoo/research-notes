...
| Johnson-Lindenstrauss (1984) | The founding paper: proves that any $n$-point subset of a Hilbert space embeds into $O(\log n / \varepsilon^2)$ dimensions with $(1\pm\varepsilon)$ distortion via a random projection | [Extensions of Lipschitz Mappings into a Hilbert Space](http://stanford.edu/class/cs114/readings/JL-Johnson.pdf) |
| Dasgupta-Gupta (2003) | Reproves the JL lemma using only chi-squared tail bounds and a union bound — the standard clean modern proof | [An Elementary Proof of a Theorem of Johnson and Lindenstrauss](https://cseweb.ucsd.edu/~dasgupta/papers/jl.pdf) |
| Achlioptas (2003) | $\pm1$ and sparse $\{-1,0,+1\}$ entries work as JL matrices, reducing arithmetic cost | [Database-Friendly Random Projections: Johnson-Lindenstrauss with Binary Coins](https://cgi.di.uoa.gr/~optas/papers/jl.pdf) |
| Ailon-Chazelle (2009) | FJLT = $PHD_x$; reduces embedding time from $O(dk)$ to $O(d\log d)$ | [The Fast Johnson-Lindenstrauss Transform and Approximate Nearest Neighbors](https://www.cs.princeton.edu/~chazelle/pubs/FJLT-sicomp09.pdf) |
| Kane-Nelson (2014) | Column sparsity $s = O(\varepsilon^{-1}\log n)$ is optimal; proves both upper and lower bounds on sparsity | [Sparser Johnson-Lindenstrauss Transforms](https://arxiv.org/abs/1012.1577) |
| Larsen-Nelson (2017) | Proves $m = \Omega(\varepsilon^{-2}\log n)$ lower bound for any (not just linear) embedding; confirms JL is tight | [Optimality of the Johnson-Lindenstrauss Lemma](https://arxiv.org/abs/1609.02094) |
| Vempala (2004) | Graduate monograph; rigorous proofs with broad algorithmic applications of random projections | [The Random Projection Method](https://bookstore.ams.org/dimacs-65) |
| Woodruff (2014) | Comprehensive survey; JL as primitive for least squares, low-rank approximation, and regression | [Sketching as a Tool for Numerical Linear Algebra](https://arxiv.org/abs/1411.4357) |
| Freksen (2021) | Self-contained survey from original 1984 lemma through sparse and fast constructions to the 2017 lower bound | [An Introduction to Johnson-Lindenstrauss Transforms](https://arxiv.org/abs/2103.00564) |
