...
> $$
> a sum of $N$ *independently supervised* terms. Producing $\log p(x_i \mid x_{<i})$ at every position $i$ requires a contextualized hidden state $h_i^{(\ell)}$ at every layer $\ell$ and every position $i$, since $h_i^{(L)}$ is the vector from which position $i$'s logits are read out. Maintaining $O(N \times d)$ activations through all $L$ layers is therefore *forced* by the fact that the model owes $N$ independent predictions — and the resulting $O(N^2)$ self-attention cost is amortized over those $N$ outputs, i.e. $O(N)$ compute per prediction.
>
